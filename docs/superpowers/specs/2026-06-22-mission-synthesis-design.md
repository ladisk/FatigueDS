...
- Matrix-inversion method (Vol. 5 §11.4.9) accounting for off-resonance response.
...
**3.3 Invert FDS → test PSD** — closed-form inverse of Lalanne's white-noise FDS
(Vol. 5 eq [4.9], validated in `validation/validation.md` to <0.05% at Q=200):
...
Narrow-band assumption (`n0+ = f0`), the same basis as `Spectrum`'s random-PSD route.
Where `D_ref(f0) = 0` or `f0 ≈ 0`, set `G(f0) = 0` (avoid division blow-up).
...
class MissionSynthesis:
    ...
    def invert(self, T_test, k=None, C=None, p=None, Q=None):
        ...
        Material params default to the (validated-consistent) event values; args override."""
...
def invert_fds_to_psd(fds_ref, f0_range, k, C, p, Q, T_test) -> np.ndarray  # eq [4.9] inverse
```

State set by the workflow: `f0_range`, `fds_ref`, `ers_ref`, `test_psd`,
`test_psd_freq`, `ers_ratio`, plus stored `k, C, p, Q, T_test`.
